                  ...
                  <div class="card-header position-relative py-2 px-3">
                    <div class="row align-items-center">
                      <div class="col-4">
                        ...
                        <div class="input-group">
                          <input v-model="batch.inputVal" @input="onInputSelectBatchProductListOrderTicket(dataShowBatchProductOrdetTicketToSelect, batch, $event.target.value)" class="form-control form-control-sm hide-input-btn px-1" type="number" min="0">
...
export default {
  ...
  methods: {
    ...
    waitForConfirmationNoQtyBatch: function () {
      return new Promise((resolve, reject) => {
        const modalNotifConfirmBatch = $('#modalNotifConfirmQtyBatch');
        modalNotifConfirmBatch.modal('show');

        modalNotifConfirmBatch.on('click', '.btn-primary', () => {
          modalNotifConfirmBatch.modal('hide');
          resolve();
        });

        modalNotifConfirmBatch.on('click', '.btn-secondary', () => {
          modalNotifConfirmBatch.modal('hide');
          reject(); // Menggunakan reject di sini
        });
      });
    },
      ...
      try{
        ...
        for(let i = 1; i <= pagesTicket; i++){
          ...
          dataProduct.forEach((data) => {
            ...
            const findExistingTicket = this.dataFilterAllTicket.find((x) => x.member_id == member_id && x.user.user_uuid == userBa);
