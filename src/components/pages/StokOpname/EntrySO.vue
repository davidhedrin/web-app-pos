<template>
  <div class="d-flex align-items-center mb-3 mt-1">
    <span class="badge rounded me-3 ms-1 badge-subtle-primary">
      <span class="fas fa-file-contract fs-2 text-primary"></span>
    </span>
    <div class="col">
      <h5 class="mb-0 text-primary position-relative">
        <span class="bg-200 dark__bg-1100 pe-3 fs-2">Entry Stock Opname</span>
        <span class="border position-absolute top-50 translate-middle-y w-100 start-0 z-n1"></span>
      </h5>
      <p class="mb-0 fs--1"><a href="javascript:void(0)" class="text-dark">Martha Tilaar Shop</a> / Entry Stock Opname</p>
    </div>
  </div>
      ...
      <div class="row flex-between-center gx-0">
        <div class="col-lg-auto d-flex align-items-center"><img class="img-fluid" src="@/assets/img/illustration/reports-greeting.png" alt="">
          <div class="ms-x1">
            ...
            <h4 class="mb-0 text-primary fw-bold"><span class="text-info fw-medium">Entry Stock Opname (SO)</span></h4>
          </div>
        </div>
  ...
  <Pages>
    ...
    <Teleport to="body">
      ...
      <FormModal
        ...
        @close2="showModal88 = false"
      >
        <template #header>
          <h3>Batch Number and Expired Date</h3>
        </template>
        <template #body>
          <div style="width: 90vw">
            <!-- <pre>{{ var_SO_awal_batch }}var_SO_awal_batch</pre>
            <pre>{{ var_so_d2 }}var_so_d2</pre> -->

            <h3>
              Total Qty:
              {{ var_SO_awal[key_total_sum_batch_awal] }}
            </h3>
            ...
            <button
              ...
              @click="tambah_batch"
            >
              <i class="fa fa-list" aria-hidden="true"></i>&nbsp;&nbsp;Add Batch
            </button>
            <br />
            <br />
            <table
            ...
            >
              <thead>
                <tr>
                  ...
                  <td class="text-center"><strong>Optional</strong></td>
                  <td class="text-center"><strong>Optional Name</strong></td>
              ...
              <tbody>
                <tr v-for="(value, key) in var_so_d2" :key="key">
                  <td>{{ value[1] }}</td>
                  <td>{{ value[2] }}</td>
                  <td>{{ value[3] }}</td>
                  <td>{{ value[4] }}</td>
                  ...
                  <td class="text-center">
                    <template
                      v-if="
                        var_SO_awal_batch[
                          value[0] + 'AABBCC' + value[4] + 'AABBCC' + value[6]
                        ].batch != '000000' && typeof value[0] === 'string'
                      "
                    >
                      <a
                        @click="
      ...
      >
        <template #header>
          <h3>Browse Data From Ticket SO</h3>
        </template>
        <template #body>
          <div style="width: 90vw">
      ...
      >
        <template #header>
          <h3>Entry Stock Opname (SO)</h3>
        </template>
        <template #body>
          <div style="width: 90vw">
            <!-- <pre>{{ var_SO_awal_batch }}</pre>
            <pre>{{ var_SO_awal_batch_final }}var_SO_awal_batch_final</pre> -->
            <!-- <pre>{{ dataTr103 }}</pre> -->
            <!-- <pre>{{ dataTr103 }}</pre>
            <pre>{{ tabel_ticket_so.length }}</pre> -->
            <!-- <pre>{{ flagSaveAwal }}</pre>
            <pre>{{ flagDownloadPDF }}</pre>
            <pre>{{ tabel_ticket_so }}</pre> -->
            ...
            <pre>{{ var_SO_awal_batch }}</pre>
            <pre>{{ var_SO_awal_batch_final }}var_SO_awal_batch_final</pre>
            <pre>{{ tabel_ticket_so_final }}tabel_ticket_so_final</pre> -->

            <!-- <pre>{{ var_SO_awal }}var_SO_awal</pre>
            <pre
              >{{ tabel_ticket_so_final_batch }}tabel_ticket_so_final_batch</pre
            >
            <pre>{{ var_so_variant }}var_so_variant</pre>
            <pre>{{ var_so_variant_fix }}var_so_variant_fix</pre>

            <pre>{{ API_split_var_SO_awal }}API_split_var_SO_awal</pre>
            <pre
              >{{
                API_split_tabel_ticket_so_final_batch
              }}API_split_tabel_ticket_so_final_batch</pre
            >
            <pre>{{ API_split_var_so_variant }}API_split_var_so_variant</pre>
            <pre>{{ var_SO_awal_qty_batch }}var_SO_awal_qty_batch</pre>
            <pre
              >{{ tabel_ticket_so_final_batch }}tabel_ticket_so_final_batch</pre
            >
            <pre
                ...
                <tbody>
                  <tr>
                    ...
                    <td class="text-success text-left">
                      <strong>
                        {{ dataTr103.docNum == "" ? "-" : dataTr103.docNum }}
                      </strong>
                    </td>
                    ...
                    <td class="text-success text-left">
                      <strong>
                        {{
                          dataTr103.docStatus == undefined
                            ? "-"
                            : dataTr103.docStatus == "O"
                            ? "Open"
                            : dataTr103.docStatus == "D"
                            ? "Draft"
                            : "Close"
                        }}</strong
                      >
                    ...
                    <td class="text-success text-left" width="10">
                      <template v-if="flagclickFromStore == true">
                        <textarea
                          ...
                          rows="3"
                          cols="40"
                          v-model="dataTr103.comments"
                          placeholder=""
                          :disabled="
                            dataTr103.docStatus == 'O' ||
                            dataTr103.docStatus == 'C'
                          "
                        ></textarea>
                      </template>
                    </td>
                ...
                <tbody>
                  <tr>
                    <td class="text-left" colspan="3">
                      ...
                      <template
                        v-if="dataTr103.docStatus == 'D' && flagViewSO == false"
                      >
                        <button
                          class="btn btn-success btn-sm me-1"
                    ...
                    <td class="text-success text-left">
                      <strong>
                        {{
                          dataTr103.storeCodeFrm == undefined
                            ? "-"
                            : dataTr103.storeCodeFrm
                        }}
                      </strong>
                    </td>
                    ...
                    <td class="text-success text-left">
                      <strong>
                        {{
                          dataTr103.storeNameFrm == undefined
                            ? "-"
                            : dataTr103.storeNameFrm
                        }}
                      </strong>
                    </td>
                  </tr>
                    ...
                    <tr>
                      ...
                      <td class="text-left">
                        <input
                          type="checkbox"
                          v-bind:value="1"
                          v-model="dataTr103.activeproduct"
                          :disabled="cboOptDetail2_status"
                        />&nbsp;&nbsp;&nbsp;Active Product
                      </td>
                      <td class="text-left">&nbsp;</td>
            ...
            <template v-if="flagGetDataSO">
              <button
                type="button"
                class="btn btn-success"
                @click="getDataSOTableRincian()"
              >
                Get Data Stok Opname
              </button>
            </template>
            ...
            <template v-if="flagGetDataSO == false">
              ...
              <div class="text-left">
                <template v-if="tabel_ticket_so.length > 0">
                  ...
                  <template v-if="trSO_update == 1">
                    <template v-if="dataTr103.docStatus == 'O'">
                      <button
                        type="button"
                        class="btn btn-success"
                        @click="saveData()"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      var_SO_awal: {},
      var_SO_awal_qty_batch: {},
      var_SO_awal_batch: {},
      var_SO_awal_batch_final: {},
      var_so_variant: {},
      var_so_variant_fix: {},
      ...
      var_so_d2: {},
      key_total_sum_batch: "",
      key_total_sum_batch_awal: "",
      tabel_ticket_so_final: {},
      tabel_ticket_so_final_batch: {},
      itemcode_data: "",

      API_split_var_SO_awal: [],
      API_split_tabel_ticket_so_final_batch: [],
      API_split_var_so_variant: [],

      API_split_temp_var_SO_awal: {},
      API_split_temp_tabel_ticket_so_final_batch: {},
      API_split_temp_var_so_variant: {},
  ...
  methods: {
    ...
    async kalkulasiTotal() {
      ...
      Object.keys(mythis.var_SO_awal).forEach(function (key) {
        if (mythis.var_SO_awal[key] == undefined) {
          return;
        }
        a += parseInt(mythis.var_SO_awal[key]);
        b += parseInt(mythis.var_so_variant[key]);
        c += parseInt(mythis.var_SO_awal_qty_batch[key]);
      });
    ...
    deleteItemTmp(key, x) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          this.var_so_d2.splice(key, 1);
          //this.var_SO_awal_batch.splice(key, 1);

          delete this.var_SO_awal_batch[x];
    ...
    async saveQtyBatch() {
      ...
      var totalx = mythis.var_SO_awal[mythis.key_total_sum_batch_awal];
      console.log(n);
      console.log(mythis.key_total_sum_batch_awal);
      console.log(totalx);
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          await Object.keys(mythis.var_SO_awal_batch).forEach(async function (
            ...
            console.log("key " + key);
            mythis.var_SO_awal_qty_batch[mythis.key_total_sum_batch_awal] =
              totalx;

            if (mythis.var_SO_awal_batch_final.hasOwnProperty(key)) {
              mythis.var_SO_awal_batch_final[key] =
                mythis.var_SO_awal_batch[key];

              const myArray = key.split("AABBCC");
              mythis.tabel_ticket_so_final[myArray[1]] = mythis.var_so_d2;
              mythis.tabel_ticket_so_final_batch[myArray[1]] =
    ...
    async tambah_batch() {
      ...
      console.log(mythis.var_so_d2[0][4]);
      console.log(mythis.var_so_d2[0][5]);
      // var_so_d2
      // var_SO_awal_batch

      var a = 0;
      var n = 0;
      Object.keys(mythis.var_SO_awal_batch).forEach(function (key) {
        n++;
        if (mythis.var_SO_awal_batch[key].batch == "XXXXXX") {
          a = 1;
        }
      });
      ...
      if (a == 0) {
        ...
        const countries = [
          "XXXXXX" + (n + 1),
          "-",
          mythis.var_so_d2[0][2],
          mythis.var_so_d2[0][3],
          mythis.var_so_d2[0][4],
          mythis.var_so_d2[0][5],
          "XXXXXX" + (n + 1),
          "2024-01-31",
          0,
          0,
        ...
        //console.log(mythis.var_so_d2.length + 1);
        mythis.var_so_d2[n] = countries;
        //////////////////////////////////////////
        mythis.var_SO_awal_batch[
          "XXXXXX" +
            (n + 1) +
    ...
    ///////////////////////////////////////////////////
    async getDataSOTableRincian_D2(idx) {
      var mythis = this;
      ...
      while (count > 0) {
        ...
        const reqData = await axios({
          ...
          url:
            mythis.$root.API_ERP +
            "/wms/getDataSOTableRincian_with_batch_byid/" +
            mythis.dataTr103.id,
          data: {
        ...
        Object.keys(resData.data).forEach(function (key) {
          mythis.key_total_sum_batch = "XXYYZZ" + resData.data[key].itemCode;
          //var itemcode_data = resData.data[key].itemCode;
          mythis.itemcode_data = resData.data[key].itemCode.trim();
          ...
          mythis.var_SO_awal_batch[
            ...
            batch: resData.data[key].batchNo,
            expiredDate: resData.data[key].expiredDate,
            stockOpnameQty: resData.data[key].stockOpnameQty,
          };

          baris = baris + 1;

          /*
          if (resData.data[key].optional != "Jumlah Item") {
            mythis.var_so_d1[
            ...
            mythis.var_so_variant[
              resData.data[key].id + "XXYYZZ" + resData.data[key].itemCode
            ] = resData.data[key].varianceQty1;
          }
          */
        });
        console.log(mythis.var_so_d1);
        ///////////////////////////////////////////////////////////////////
        // if (resData.data.length > 0) {
        //   Object.keys(resData.data).forEach(function (key) {
      ...
      console.log("total ticket so " + mythis.tabel_ticket_so.length);
      mythis.$root.loader = false;
      mythis.flagGetDataSO = false;

      // if (mythis.tabel_ticket_so_final.hasOwnProperty(mythis.itemcode_data)) {
      //   mythis.var_so_d2 = mythis.tabel_ticket_so_final[mythis.itemcode_data];
      //   mythis.var_SO_awal_batch =
      //     mythis.tabel_ticket_so_final_batch[mythis.itemcode_data];
      // }

      if (
        mythis.dataTr103.approval_at == "" ||
        mythis.dataTr103.approval_at == null
      ) {
    ...
    confirmTicket() {
      ...
      Swal.fire({
        title: "Confirm?",
        text: "Apakah Anda ingin mengkonfirmasi SO ini? dan tidak bisa mengulang SO Entry?",
        icon: "question",
        ...
        if (result.isConfirmed) {
          ...
          axios
            .put(
              mythis.$root.API_ERP +
                "/wms/confirmTicket_SO_SO_Entry/" +
                mythis.dataTr103.id,
              {
                dataTr103: mythis.dataTr103,
            ...
            .then((res) => {
              ...
              Swal.fire(
                "Update Success!",
                "SO Entry telah dikonfirmasi sesuai. Silakan lanjutkan proses Adjustment SO",
                "success"
              );
              mythis.$root.loader = false;
              mythis.dataTr103.docStatus = "O";
              mythis.close();
              mythis.close21();
              mythis.refreshTable();
            })
    ...
    async postSplitDataSO_batch() {
      ...
      try {
        ...
        Object.entries(mythis.var_SO_awal).forEach((entry) => {
          ...
          mythis.API_split_temp_var_SO_awal[key] = value;
          if (i >= limitx) {
            key_split++;
            i = 0;
            mythis.API_split_var_SO_awal[key_split - 2] =
              mythis.API_split_temp_var_SO_awal;
            mythis.API_split_temp_var_SO_awal = {};
          }
        });
        ...
        //////////////////////////////////////////////////////////////
        key_split = 1;
        Object.entries(mythis.var_so_variant).forEach((entry) => {
          const [key, value] = entry;
          console.log(key, value);
          i++;

          //mythis.API_split_temp_var_so_variant.push({ key: value });
          mythis.API_split_temp_var_so_variant[key] = value;
          if (i >= limitx) {
            key_split++;
            i = 0;
            mythis.API_split_var_so_variant[key_split - 2] =
              mythis.API_split_temp_var_so_variant;
            mythis.API_split_temp_var_so_variant = {};
          }
        });
        ...
        //////////////////////////////////////////////////////////////
        key_split = 1;
        Object.entries(mythis.tabel_ticket_so_final_batch).forEach((entry) => {
          const [key, value] = entry;
          console.log(key, value);
          i++;

          mythis.API_split_temp_tabel_ticket_so_final_batch[key] = value;
          if (i >= limitx) {
            key_split++;
            i = 0;
            mythis.API_split_tabel_ticket_so_final_batch[key_split - 2] =
        ...
        for (let i = 0; i < d; i++) {
          const reqData = await axios({
            method: "post",
            url: mythis.$root.API_ERP + "/wms/saveData_SO_SO/",
            data: {
              dataTr103: mythis.dataTr103,
              API_split_var_SO_awal: mythis.API_split_var_SO_awal[i],
              API_split_var_so_variant: mythis.API_split_var_so_variant[i],
              userid: mythis.userid,
            },
          });
          //const resData = reqData.data;
        ...
        var aaa =
          Object.keys(mythis.tabel_ticket_so_final_batch).length / limitx;
        let ddd = Math.ceil(aaa);

        for (let i = 0; i < ddd; i++) {
          const reqData = await axios({
            method: "post",
            url: mythis.$root.API_ERP + "/wms/saveData_SO_SO_batch/",
            data: {
              dataTr103: mythis.dataTr103,
              API_split_tabel_ticket_so_final_batch:
                mythis.API_split_tabel_ticket_so_final_batch[i],
              userid: mythis.userid,
            },
          });
          //const resData = reqData.data;
    ...
    async postSplitDataSO() {
      ...
      console.log(mythis.var_SO_awal);
      console.log(Object.keys(mythis.var_SO_awal).length);
      console.log(limitx);
      ...
      try {
        ...
        /*
        //kasino dimatikan dulu karena pindah tombol
        var aaa =
          Object.keys(mythis.tabel_ticket_so_final_batch).length / limitx;
        let ddd = Math.ceil(aaa);
      ...
      Swal.fire(
        "Update Success!",
        "Data telah disimpan sementara. Silakan lanjutkan proses Confirm SO untuk dapat melanjutkan proses Adjustment",
        "success"
      );
      ...
      if (
        mythis.dataTr103.id == "" ||
        mythis.dataTr103.id == null ||
        mythis.dataTr103.id == undefined
      ) {
        mythis.close();
        mythis.close21();
      }
    ...
    saveData() {
      ...
      if (beda == 1) {
        Swal.fire(
          "Failed!",
          "Ada perbedaan antara Total Qty SO dengan Total Qty Batch, silakan periksa kembali",
          "error"
        );
        return false;
      }
      ...
      Swal.fire({
        title: "Simpan?",
        text: "Apakah Anda ingin menyimpan input SO ini?",
        icon: "question",
    ...
    async getDataSOTableRincian() {
      ...
      while (count > 0) {
        ...
        const reqData = await axios({
          ...
          url:
            mythis.$root.API_ERP +
            "/wms/getDataSOTableRincian_non_batch/" +
            mythis.dataTr103.id,
          data: {
            data: mythis.dataTr103,
            limit: limitx,
        ...
        columns: [
          ...
          },
          {
            ...
            formatter: (_, row) =>
              row.cells[3].data == "Jumlah Item"
                ? html(
                    `<input id="total_a" type="number" value="` +
                      this.total_a +
                ...
                : html(
                    "<strong>" +
                      ...
                      // row.cells[7].data +
                      this.var_SO_awal[
                        row.cells[0].data + "XXYYZZ" + row.cells[4].data
                      ] +
                      '" type="number" pattern="[0-9]" onkeydown="javascript: return event.keyCode == 69 ? false : true" style="width: 6em" min="0" ' +
                      ' data-id="' +
                      row.cells[0].data +
                      "XXYYZZ" +
                      row.cells[4].data +
                      '" ' +
                      (this.dataTr103.docStatus == "C" ? " readonly" : "") +
                      ">" +
                      "</strong>"
                  ),
          },
          {
            ...
            formatter: (_, row) =>
              row.cells[3].data == "Jumlah Item"
                ? html(
                    `<input id="total_b" type="number" value="` +
                      this.total_b +
                      `" pattern="[0-9]" style="width: 6em" readonly>`
                  )
                : this.var_so_variant[
                ...
                ? html(
                    "<div'>" +
                      ...
                      '" pattern="[0-9]" style="width: 6em;background-color:#ffcccc" readonly />' +
                      "</div>"
                  )
                : html(
                    "<div'>" +
...

          {
            id: "QtyBatch",
            name: html(
              '<div style="padding: 5px;border-radius: 5px;text-align: center;"><b>Total Qty Batch</b></div>'
            ),
            formatter: (_, row) =>
              row.cells[3].data == "Jumlah Item"
                ? html(
                    `<input id="total_c" type="number" value="` +
                      this.total_c +
                      `" pattern="[0-9]" style="width: 6em" readonly>`
                  )
                : html(
                    "<div'>" +
                      ...
                      row.cells[4].data +
                      '" type="number" value="' +
                      //row.cells[8].data +//var_SO_awal_qty_batch
                      this.var_SO_awal_qty_batch[
                        row.cells[0].data + "XXYYZZ" + row.cells[4].data
                      ] +
                      '" pattern="[0-9]" style="width: 6em" readonly /><input style="display:none" id="variant_fix_' +
                      row.cells[0].data +
                      "XXYYZZ" +
                      row.cells[4].data +
                      '" type="text" value="' +
                      parseInt(
                        this.var_so_variant_fix[
                          row.cells[0].data + "XXYYZZ" + row.cells[4].data
                        ]
                      ) +
                      '" readonly />' +
...

          {
            ...
            formatter: (_, row) =>
              row.cells[3].data == "Jumlah Item"
                ...
                ? html(
                    `
                    <div style="display:none" id="input_batch_` +
                      row.cells[0].data +
                      "XXYYZZ" +
                      row.cells[4].data +
                      `" >
                    <button
                      id="input_batch"
                  data-id="${row.cells[0].data}XXYYZZ${row.cells[4].data}" class="btn btn-sm btn-warning text-white" data-toggle="tooltip" title="View" > <i  class="fa fa-list" aria-hidden="true"></i></button>` +
                      `</div>`
                  )
                : html(
                    `
    ...
    async getDataTO_pdf_batch() {
      ...
      while (count > 0) {
        ...
        const reqData = await axios({
          ...
          url:
            mythis.$root.API_ERP +
            "/wms/getDataSOTableRincian_with_batch/" +
            mythis.dataTr103.id,
          data: {
            data: mythis.dataTr103,
            limit: limitx,
    ...
    async exportToPDFConfirm_batch() {
      ...
      var xxx =
        '<style> p, span, table { font-size: 10px } table { width: 100%; } .borderx { border-bottom: 1px solid black; border-top: 1px solid black; } .bordery { border-top: 1px solid black; } .borderyd { border-top: 1px dotted black;}</style> <h4>STOCK OPNAME WITH BATCH</h4><table cellpadding="0"> <tr> <th width="10%">Doc Num</th> <th width="25%">: ' +
        this.dataTr103.docNum +
        '</th> <th width="10%">Doc Status</th> <th width="20%">: ' +
        (this.dataTr103.docStatus == "O" ? "Open" : "Close") +
        '</th> <th width="12%">Description</th> <th width="30%">: ' +
        this.dataTr103.description +
        "</th> </tr> <tr> <th>Remarks</th> <th>: " +
        this.dataTr103.comments +
        "</th> <th>Reference</th> <th>: " +
        this.dataTr103.reference +
        "</th> <th>Doc Date</th> <th>: " +
        this.dataTr103.docDate +
        // '</th> </tr> <tr> <th colspan="7" class="bordery"><br/></th></tr><tr> <th>From Store</th> <th> ' +
        // "&nbsp;" +
        // "</th> <th>From Store ID</th> <th>: " +
        // this.dataTr103.storeCode +
        // "</th> <th>From Store Name</th> <th>: " +
        // this.dataTr103.storeName +
        ...
        this.dataTr103.storeCodeTo +
        "</th> <th>SO Store Name</th> <th>: " +
        this.dataTr103.storeNameTo +
        "</th> </tr> <tr> <th>WHS Code</th> <th>: " +
    ...
    async exportToPDFConfirm() {
      ...
      var xxx =
        '<style> p, span, table { font-size: 10px } table { width: 100%; } .borderx { border-bottom: 1px solid black; border-top: 1px solid black; } .bordery { border-top: 1px solid black; } </style> <h4>STOCK OPNAME NON BATCH</h4><table cellpadding="0"> <tr> <th width="10%">Doc Num</th> <th width="25%">: ' +
        this.dataTr103.docNum +
        '</th> <th width="10%">Doc Status</th> <th width="20%">: ' +
        (this.dataTr103.docStatus == "O" ? "Open" : "Close") +
        '</th> <th width="12%">Description</th> <th width="30%">: ' +
    ...
    getRincianHeaderTR_NON_Update(id) {
      ...
      mythis.$root.loader = true;
      axios
        .get(this.$root.API_ERP + "/wms/getRincianheaderIDUpdate_SO/" + id)
        .then((res) => {
          //   mythis.dataRincianHeaderID = res.data.data[0];
          //   mythis.dataRincianHeaderIDTR = res.data.data[0];
          mythis.dataTr103.id = res.data.data[0].id;
          mythis.dataTr103.docNum = res.data.data[0].docNum;
          //mythis.dataTr103.docNum_SO = null;

          mythis.dataTr103.approval_at = res.data.data[0].approval_at;

          mythis.dataTr103.docStatus = res.data.data[0].docStatus;
          mythis.dataTr103.docDate = res.data.data[0].docDate1;
          mythis.dataTr103.reference = res.data.data[0].reference;
          mythis.dataTr103.description = res.data.data[0].description;
          mythis.dataTr103.comments = res.data.data[0].comments;

          mythis.dataTr103.storeCodeFrm = res.data.data[0].storeCodeFrm;
          mythis.dataTr103.whsCodeFrm = res.data.data[0].whsCodeFrm;
          mythis.dataTr103.priceCodeFrm = res.data.data[0].priceCodeFrm;
          mythis.dataTr103.discCodeFrm = res.data.data[0].discCodeFrm;

          mythis.dataTr103.storeCodeTo = res.data.data[0].storeCodeTo;
          mythis.dataTr103.whsCodeTo = res.data.data[0].whsCodeTo;
          mythis.dataTr103.priceCodeTo = res.data.data[0].priceCodeTo;
          mythis.dataTr103.discCodeTo = res.data.data[0].discCodeTo;

          mythis.dataTr103.outSts = res.data.data[0].outSts;
          mythis.dataTr103.inSts = res.data.data[0].inSts;

          mythis.dataTr103.itemCount = res.data.data[0].itemCount;
          mythis.dataTr103.quantityAmont = res.data.data[0].quantityAmont;

          mythis.dataTr103.storeNameFrm = res.data.data[0].storeCodeFrm1;
          mythis.dataTr103.storeNameTo = res.data.data[0].storeCodeTo1;

          mythis.dataTr103.adminsamaso = res.data.data[0].adminsamaso;
          mythis.dataTr103.activeproduct = res.data.data[0].activeproduct;

          mythis.dataTr103.optCode = res.data.data[0].optCode;
          mythis.dataTr103.optDetail = res.data.data[0].optDetail;
    ...
    getRincianHeaderTR_Update(id) {
      ...
      axios
        .get(
          this.$root.API_ERP +
            "/wms/getRincianheaderIDUpdate_SO_TrSO/" +
            id
        )
        .then((res) => {
          //   mythis.dataRincianHeaderID = res.data.data[0];
          ...
          mythis.dataTr103.id = res.data.data[0].id;
          mythis.dataTr103.docNum = res.data.data[0].docNum;
          mythis.dataTr103.docNum_SO = res.data.data[0].docNum;
          ...
          console.log(mythis.dataTr103.approval_at);
          if (
            mythis.dataTr103.approval_at == "" ||
            mythis.dataTr103.approval_at == null
          ) {
    ...
    getFlagUpdate() {
      ...
      this.flagSave = 0;
      this.flagSdataTr103_commentsave = "";
    },
    modal88() {
    ...
    close: function () {
      ...
      this.flagDownloadPDF = 0;
      ///////////////////////////////////////////
      this.dataTr103.adminsamaso = false;
      this.dataTr103.activeproduct = false;
      ...
      this.tabel_ticket_so_final = {};
      this.tabel_ticket_so_final_batch = {};
      ...
      this.var_so_variant = {};
      this.var_so_variant_fix = {};
      this.key_total_sum_batch = "";
      this.key_total_sum_batch_awal = "";
      this.var_SO_awal_batch = {};
      this.var_SO_awal_batch_final = {};
      this.itemcode_data = "";
      ///////////////////////////////////////////

      this.API_split_var_SO_awal = [];
      this.API_split_tabel_ticket_so_final_batch = [];
      this.API_split_var_so_variant = [];

      this.API_split_temp_var_SO_awal = {};
      this.API_split_temp_tabel_ticket_so_final_batch = {};
      this.API_split_temp_var_so_variant = {};
    ...
    jqueryDelEdit() {
      ...
      $(document).on("click", "#input_batch", function () {
        ...
        mythis.getDataSOTableRincian_D2(id);
        mythis.modal88();
      });
      ...
      $(document).on("change", "input", function () {
        ...
        let result = id.includes("XXYYZZ");
        let result2 = id.includes("AABBCC");
        console.log(result);
        console.log(result2);
        if (result == true) {
          //non batch
          ...
          //alert(id + x_value);
          console.log(x_value + " " + x_value2);
          console.log(mythis.var_so_d1[id]);
          var variantx = x_value - mythis.var_so_d1[id];
          var variantx_fix = x_value - mythis.var_SO_awal_qty_batch[id]; //kz
          $("#variant_" + id).val(variantx);

          mythis.var_so_variant[id] = variantx;
          mythis.var_SO_awal[id] = x_value; // set value hasil SO

          mythis.var_so_variant_fix[id] = variantx_fix;
          $("#variant_fix_" + id).val(variantx_fix);
          if (variantx_fix != 0) {
            $("#input_batch_" + id).show();
          } else {
            $("#input_batch_" + id).hide();
          }
          ...
          console.log("masuk " + x_value);
          //let x_value2 = $(id).val();
          //alert(id + x_value);

          if (id.includes("d2_batch_")) {
            let idx = id.replace("d2_batch_", "");

            mythis.var_SO_awal_batch[idx].batch = x_value;
          } else if (id.includes("d2_soqty_")) {
            let idx = id.replace("d2_soqty_", "");
            mythis.var_SO_awal_batch[idx].stockOpnameQty = x_value;
          }
        }
      ...
      $(document).on("click", "#changeLock", function () {
        ...
        if (mythis.dataTr103.docStatus == "C") return false;
    ...
    getTable() {
      ...
      this.grid2.updateConfig({
        ...
        server: {
          url: this.$root.API_ERP + "/wms/pagingTransOut_SO_Entry",
          then: (data) =>
            data.results.map((card) => [
              card.id,
              //"",
              ...
              `
                ...
                : card.docStatus == "O" && card.inSts == null
                ? html(
                    `
              <button data-id="${card.id}" class="btn btn-sm btn-info text-white" id="viewToTinUpdate" data-toggle="tooltip" title="View" ><i class="fa-solid fa-pen-to-square"></i> Entry Data</button>
              `
                  )
                : html(
                    `
    ...
    getpagingTransOutfromTicket() {
      ...
      this.grid11.updateConfig({
        ...
        server: {
          url: this.$root.API_ERP + "/wms/pagingTransOutfromTicket_SO",
          then: (data) =>
            data.results.map((card) => [
              card.id,
              "",
    ...
    getRincianHeaderID(id) {
      ...
      axios
        ...
        .then((res) => {
          mythis.dataRincianHeaderID = res.data.data[0];
          mythis.dataTr103.storeCode = res.data.data[0].storeCode;
          mythis.dataTr103.storeName = res.data.data[0].storeName;
          mythis.dataTr103.docStatus = "O";
          mythis.dataTr103.reference = res.data.data[0].docNum;
          mythis.dataTr103.comments = res.data.data[0].comments;
          mythis.dataTr103.docDate = new Date().toISOString().slice(0, 10);
          //mythis.dataTr103.reference = res.data.data[0].poNo;
          mythis.dataTr103.whsCode = res.data.data[0].whsCode;
          mythis.tmp_getCboWhsCodeInventoryOptions =
            res.data.data[0].whsCode + " - " + res.data.data[0].whsName;
          mythis.dataTr103.comments = "";
          mythis.dataTr103.whsName = "";
          mythis.dataTr103.priceCode = res.data.data[0].priceCode;
          mythis.tmp_getCboPriceCodeInventoryOptions =
            res.data.data[0].priceName;
          mythis.dataTr103.priceName = "";
          mythis.dataTr103.discCode = res.data.data[0].discCode;
          mythis.tmp_getCboDiscCodeInventoryOptions = res.data.data[0].discName;
          mythis.dataTr103.discName = "";
          mythis.$root.loader = false;

          mythis.dataTr103.storeCodeTo = res.data.data[0].storeCodeTo;
          mythis.dataTr103.storeNameTo = res.data.data[0].storeName2;
          mythis.dataTr103.priceCodeTo = res.data.data[0].priceCodeTo;
          mythis.dataTr103.discCodeTo = res.data.data[0].discCodeTo;
          mythis.dataTr103.whsCodeTo = res.data.data[0].whsCodeTo;
    ...
    mySelectEvent2() {
      ...
      this.dataTr103.whsCode = this.tmp_getCboWhsCodeInventoryOptions.code;
    },

    mySelectEvent22() {
      this.dataTr103.priceCode = this.tmp_getCboPriceCodeInventoryOptions.code;
    },

    mySelectEvent222() {
      this.dataTr103.discCode = this.tmp_getCboDiscCodeInventoryOptions.code;
    },
    savePenerimaanBarang() {
      var mythis = this;

      if (
        mythis.dataTr103.discCode == null ||
        mythis.dataTr103.discCode == "" ||
        mythis.dataTr103.discCode == undefined
      ) {
      ...
      if (
        mythis.dataTr103.whsCode == null ||
        mythis.dataTr103.whsCode == "" ||
        mythis.dataTr103.whsCode == undefined
      ) {
      ...
      if (
        mythis.dataTr103.priceCode == null ||
        mythis.dataTr103.priceCode == "" ||
        mythis.dataTr103.priceCode == undefined
      ) {
